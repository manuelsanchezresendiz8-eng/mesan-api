...
class PolicyAuditEngine:
    ...
    def auditar(self, data: dict) -> dict:
        ...
        if not data.get("nom_035", False):
            ...
            score -= impacto; exposicion += 50000
            riesgos.append({"riesgo":"NOM-035","severidad":"ALTA","impacto":"MULTAS_STPS"})
            recomendaciones.append("Implementar politica de prevencion de riesgos psicosociales NOM-035")

        if not data.get("reglamento_interno", False):
            score -= 15; exposicion += 25000
            riesgos.append({"riesgo":"REGLAMENTO_INTERNO","severidad":"MEDIA","impacto":"RIESGO_LABORAL"})
            recomendaciones.append("Actualizar reglamento interno de trabajo")

        if data.get("empleados_remotos", 0) > 0 and not data.get("politica_teletrabajo", False):
            score -= 15; exposicion += 30000
            riesgos.append({"riesgo":"TELETRABAJO","severidad":"MEDIA","impacto":"CONTINGENCIA_LABORAL"})
            recomendaciones.append("Implementar politica de teletrabajo conforme a LFT")

        if not data.get("clausula_confidencialidad", False):
            score -= 10; exposicion += 15000
            riesgos.append({"riesgo":"CONFIDENCIALIDAD","severidad":"MEDIA","impacto":"RIESGO_LEGAL"})
            recomendaciones.append("Agregar clausulas de confidencialidad a contratos")

        if not data.get("cumplimiento_stps", False):
            score -= 20; exposicion += 100000
            riesgos.append({"riesgo":"STPS","severidad":"CRITICA","impacto":"MULTAS_Y_SUSPENSIONES"})
            recomendaciones.append("Regularizar cumplimiento ante STPS")

        if not data.get("plan_capacitacion", False):
            score -= 10; exposicion += 20000
            riesgos.append({"riesgo":"CAPACITACION","severidad":"MEDIA","impacto":"OBSERVACIONES_STPS"})
            recomendaciones.append("Registrar plan de capacitacion ante STPS")
